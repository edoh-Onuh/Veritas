...
3. **Challenger** stakes to dispute a claim while its challenge window is open. A committee of re-runners each re-runs the engine on the committed inputs and submits the score it derives; the claim settles when a quorum reports the *same* score. (On the deployment below that committee is **one key**, so what runs today demonstrates the mechanism, not the independence.) An implausible claim gets slashed, the challenger is rewarded, and a portable integrity score is written for other programs to read. A claim nobody disputes returns its bond once the window closes.
